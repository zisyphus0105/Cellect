...
from envs.action_observation import Cellect_Env

register_env("Cellect_Env_v0", lambda config:Cellect_Env(config))
...
def rollout(agent, env_name, num_steps, out="assdf", no_render=True):
    ...
    env = Cellect_Env(env_config=env_config)
    ...
    while rollout_steps < args.num_val_specs:
        ...
        reward_total = 0.0
        steps=0
        #print("state in the rollout: ", state)
        while not done and steps < args.traj_len:
            ...
            # print("action in the rollout: ", action[0][0])
            ...
            # print("action after change: ", action)
            # print("type: ", type(action[0]))
        ...
        ideal_spec = unlookup(norm_ideal_spec[1], norm_spec_ref[1])
        print("ideal_spec during rollout", ideal_spec)
        if done == True:
            reached_spec += 1
        ...
        pickle.dump(obs_reached, open("cellect_obs_reached_test","wb"))
        pickle.dump(obs_nreached, open("cellect_obs_nreached_test","wb"))
