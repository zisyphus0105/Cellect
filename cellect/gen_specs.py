...
def gen_data(CIR_YAML, env, num_specs):
  ...
  for key,value in specs_range.items():
      ...
      i+=1
  with open("cellect/gen_specs/synthesis_specs_gen_"+env, 'wb') as f:
...
def main():
  ...
  CIR_YAML = "circuit_deisgn/synthesis/synthesis_inputs/yaml_files/dc.yaml"
  
  gen_data(CIR_YAML, "cellect", int(args.num_specs))
